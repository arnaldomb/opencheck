...
export async function eventosRoutes(app: FastifyInstance) {
  ...
  app.post('/:id/enviar-whatsapp', async (request, reply) => {
    ...
    const { getEvoGoConfig, sendWhatsAppText, sendWhatsAppMedia } = await import('../../infra/evogo/evogo.service.js')
    ...
    if (!cfg?.whatsappDestino) {
      return reply.status(400).send({ error: 'Número de destino não configurado. Configure em Configurações → Notificações.' })
    }
    ...
    await sendWhatsAppText(eConfig, cfg.whatsappDestino, text)

    const snapshot = await prisma.snapshot.findFirst({ where: { eventoId: id } })
    if (snapshot?.imageUrl) {
      await sendWhatsAppMedia(eConfig, cfg.whatsappDestino, snapshot.imageUrl, '📸 Snapshot do evento').catch(() => {})
    }
